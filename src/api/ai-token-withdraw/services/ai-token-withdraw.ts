...
export default factories.createCoreService('api::ai-token-withdraw.ai-token-withdraw' as any, ({ strapi }) => ({
  async createWithdraw(ctx) {
    ...
    return await strapi.db.transaction(async ({ trx }) => {
      ...
      if (Number(balance.aiTokenBalance) < Number(amount)) {
        throw new Error('AI代币余额不足');
      }
      ...
      await strapi.query('api::wallet-balance.wallet-balance').update({
        where: { user: userId },
        data: { 
          aiTokenBalance: String(Number(balance.aiTokenBalance) - Number(amount)) 
        }
      });

      // 创建提现请求
      const withdrawRequest = await strapi.query('api::ai-token-withdraw.ai-token-withdraw').create({
        data: {
          amount,
          toAddress,
          status: 'pending',
  ...
  async getWithdrawHistory(userId) {
    return await strapi.query('api::ai-token-withdraw.ai-token-withdraw').findMany({
      where: { user: userId },
  ...
  async updateWithdrawStatus(id, status, txHash = null) {
    return await strapi.query('api::ai-token-withdraw.ai-token-withdraw').update({
      where: { id },
      data: { 
        status,
        txHash,
